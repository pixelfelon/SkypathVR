...
{
    public class MjpegDecoderRtsp : MjpegDecoder
    {
        ...
        public new byte[] CurrentFrame { get; private set; }

        // used to marshal back to UI thread
        private SynchronizationContext _context;

        // event to get the buffer above handed to you
        public new event EventHandler<FrameReadyEventArgs> FrameReady;
        public new event EventHandler<ErrorEventArgs> Error;

        public MjpegDecoderRtsp()
        {
            _context = SynchronizationContext.Current;
        }

        public new void ParseStream(Uri uri)
        {
            RtspClient rtsp = new RtspClient(uri);
            rtsp.Connect();

            // asynchronously get a response
            rtsp.OnResponse += OnGetResponse;
        }

        private void OnGetResponse(RtspClient sender, RtspMessage request, RtspMessage response)
        {
            ...
            try
            {
                ...
                string boundary = resp.GetHeader("Content-Type").Split('=')[1].Replace("\"", "");
